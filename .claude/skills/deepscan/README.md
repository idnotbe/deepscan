...
│   ├── models.py                 # Pydantic schemas (~230 LOC)
│   ├── constants.py              # Shared constants, SAFE_BUILTINS (~360 LOC)
│   ├── deepscan_engine.py        # Main engine + CLI (~1770 LOC)
│   ├── subagent_prompt.py        # Sub-agent prompts + sanitization (~450 LOC)
│   ├── aggregator.py             # Result aggregation + FINAL markers (~680 LOC)
│   ├── checkpoint.py             # Checkpoint management (~340 LOC)
│   ├── cancellation.py           # Work cancellation, double-tap Ctrl+C (~500 LOC)
│   ├── incremental.py            # Delta analysis, file hashing (~550 LOC)
│   ├── error_codes.py            # Error code system, 31 codes (~460 LOC)
│   ├── ast_chunker.py            # Semantic chunking with tree-sitter (~1000 LOC)
│   ├── state_manager.py          # State persistence, session lifecycle (~820 LOC)
│   ├── repl_executor.py          # Subprocess REPL with resource limits (~310 LOC)
│   ├── helpers.py                # REPL helper functions factory (~730 LOC)
│   ├── walker.py                 # Directory traversal, tree view (~430 LOC)
│   ├── grep_utils.py             # ReDoS-protected grep, process isolation (~170 LOC)
│   └── progress.py               # Progress streaming, escalation logic (~240 LOC)
├── docs/
...
- `deepscan_engine.py`: ~1770 LOC (StateManager + REPL + CLI + MAP/REDUCE)
- `subagent_prompt.py`: ~450 LOC (Embedded agent prompts + sanitization)
- `aggregator.py`: ~680 LOC (Result aggregation + FINAL markers)
- `ast_chunker.py`: ~1000 LOC (Semantic chunking with AST)
- `incremental.py`: ~550 LOC (Delta analysis)
- `cancellation.py`: ~500 LOC (Work cancellation)
- `error_codes.py`: ~460 LOC (Error handling)
- `checkpoint.py`: ~340 LOC (Checkpoint management)
- `models.py`: ~230 LOC (Pydantic schemas)
- `state_manager.py`: ~820 LOC (State persistence)
- `helpers.py`: ~730 LOC (REPL helper functions)
- `constants.py`: ~360 LOC (Shared constants)
- `repl_executor.py`: ~310 LOC (Subprocess REPL)
- `walker.py`: ~430 LOC (Directory traversal)
- `progress.py`: ~240 LOC (Progress streaming)
- `grep_utils.py`: ~170 LOC (ReDoS-protected grep)
- **Total**: ~9040 LOC (excluding tests)
